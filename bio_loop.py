...
def process_spikes_to_grid(spike_matrix: np.ndarray) -> np.ndarray:
    ...
    Args:
        spike_matrix (np.ndarray): Spike frequencies of shape [frame_count, 132]
        
    Returns:
        np.ndarray: Continuous prediction vector of shape [64]
    """
...
def calculate_centroid(latent_vector: np.ndarray, grid_size: int = 8) -> np.ndarray:
    ...
    """
    # Power-sharpening (T=1/8) to amplify the peak and suppress uniform background
    eps = 1e-8
...
class BioWorldModelLoop:
    ...
    def __init__(self, bnn_client, use_boundary_penalty: bool = True, use_velocity_decoding: bool = True, gain_factor: float = 1.0):
        ...
        # Velocity heads (columns 2 & 3) initialize to 0.0, to be learned online

    def predict_and_learn(self, z_curr: np.ndarray, action: int, z_next: np.ndarray, y_target: np.ndarray, boundary_penalty: bool = False) -> np.ndarray:
        """
        ...
        self.bnn.update_weights(z_curr_oh, action, z_next_oh, lr=0.25)
